...
def read_predicted_labels(graph_dir, corrected_chains_dir):
    """
    读取Graph文件夹和Corrected_chains文件夹中的所有文件，获取预测标签
    Corrected_chains中的标签优先级高于Graph文件夹中的标签
    
    Args:
        graph_dir: Graph文件夹路径
        corrected_chains_dir: Corrected_chains文件夹路径
    ...
    # 然后读取Corrected_chains文件夹中的标签，覆盖Graph中的标签
    # 遍历文件夹中的所有JSON文件
    for filename in os.listdir(corrected_chains_dir):
        if not filename.endswith('.json'):
            continue
        
        # 获取album_id
        album_id = filename[:-5]  # 去除.json后缀
        
        # 读取文件
        file_path = os.path.join(corrected_chains_dir, filename)
        with open(file_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        ...
        for chain in data:
            ...
            # 确保关系是数字标签
            def ensure_numeric_label(rel):
                if isinstance(rel, str):
                    return str_to_label.get(rel, -1)
                else:
                    return int(rel)
            
            ab_rel = ensure_numeric_label(ab_rel)
            bc_rel = ensure_numeric_label(bc_rel)
            ac_rel = ensure_numeric_label(ac_rel)
            
            # 只处理有效的标签
            if ab_rel != -1:
                # 处理AB关系
                person1, person2 = (a, b) if a < b else (b, a)
                key = (album_id, ab_photo, person1, person2)
                predicted_labels[key] = ab_rel
            
            if bc_rel != -1:
                # 处理BC关系
                person1, person2 = (b, c) if b < c else (c, b)
                key = (album_id, bc_photo, person1, person2)
                predicted_labels[key] = bc_rel
            
            if ac_rel != -1:
                # 处理AC关系
                person1, person2 = (a, c) if a < c else (c, a)
                key = (album_id, ac_photo, person1, person2)
                predicted_labels[key] = ac_rel
...
def main():
    ...
    # graph_dir = '/home/hello/glq/code/Album_social_relation/Convert_image_to_Graph/Graph'
    # corrected_chains_dir = '/home/hello/glq/code/Album_social_relation/Convert_image_to_Graph/Corrected_chains'

    graph_dir = '/home/hello/glq/code/Album_social_relation/Convert_image_to_Graph/Robust_testing/Robust_VLLM'
    corrected_chains_dir = '/home/hello/glq/code/Album_social_relation/Convert_image_to_Graph/Robust_testing/Corrected_chains_robust'
    ...
    # 读取预测标签（先读取Graph，再用Corrected_chains覆盖）
    print(f"\n正在读取Graph文件夹: {graph_dir}")
    print(f"正在读取Corrected_chains文件夹: {corrected_chains_dir}")
    predicted_labels = read_predicted_labels(graph_dir, corrected_chains_dir)
